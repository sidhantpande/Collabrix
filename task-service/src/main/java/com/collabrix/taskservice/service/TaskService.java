...
public class TaskService {
    ...
    public TaskService(
            ...
            TaskListRepository taskListRepository,
            WorkspaceClient workspaceClient,
            com.collabrix.taskservice.client.UserServiceClient userServiceClient,
            TaskEventPublisher taskEventPublisher
    ) {
        this(
                taskRepository,
    ...
    public TaskResponseDTO createTask(UUID workspaceId, UUID listId, UUID authId, CreateTaskRequest request) {
        ...
        Task task = Task.create(list, workspaceId, request.getTitle(),
                request.getPriority() != null ? request.getPriority() : com.collabrix.taskservice.model.enums.TaskPriority.MEDIUM,
                authId);
    ...
    public TaskResponseDTO updateTask(UUID workspaceId, UUID taskId, UUID authId, UpdateTaskRequest request) {
        ...
        if (request.getStatus() != null) {
            ...
            if (request.getStatus() == com.collabrix.taskservice.model.enums.TaskStatus.COMPLETED && task.getCompletedAt() == null) {
                task.setCompletedAt(java.time.LocalDateTime.now());
            } else if (request.getStatus() != com.collabrix.taskservice.model.enums.TaskStatus.COMPLETED) {
        ...
        taskRepository.save(task);
        if (task.getStatus() == com.collabrix.taskservice.model.enums.TaskStatus.COMPLETED && task.getAssigneeAuthId() != null && !task.getAssigneeAuthId().equals(authId)) {
